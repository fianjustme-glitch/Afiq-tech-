...
export function TabInletKnalpot({ cylinders, totalCc, bore }: any) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="border border-red-900/30 rounded-xl bg-[#0a0a0a]">
        <div className="p-6 border-b border-white/5">
          <h3 className="text-xs font-mono text-red-500 mb-2 tracking-widest uppercase">REKOMENDASI SPEK INLET</h3>
          ...
          <span className="text-[10px] text-red-600 font-mono tracking-widest block uppercase mb-4">KELOMPOK: {cylinders} SILINDER (UNIVERSAL)</span>

          <div className="space-y-2">
             {catalog.map((item, i) => {
               const isRec = i === recIndex;
               return (
                 <div key={i} className={cn("flex justify-between items-center p-4 rounded bg-[#111] border transition-colors", isRec ? "border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.2)] bg-red-950/20" : "border-white/5")}>
                   <div>
                   ...
                   {isRec && (
                     <div className="px-3 py-1 bg-red-500 text-white text-[10px] font-mono font-bold rounded">
                       REKOMENDASI
                     </div>
                   )}
                 </div>
      ...
      <div>
        <h3 className="text-xs font-mono text-red-500 mb-4 tracking-widest uppercase">DETAIL KATALOG EXHAUST</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
           {catalog.map((item, i) => {
             const isRec = i === recIndex;
             return (
               <div key={i} className={cn("rounded-lg bg-[#111] overflow-hidden border-t-2 h-full flex flex-col transition-all", item.color, isRec ? "ring-2 ring-red-500/50" : "")}>
                  <div className="p-4 bg-gradient-to-b from-[#1a1a1a] to-[#111]">
                     <div className="flex justify-between items-start">
                       ...
                       {isRec && <CheckCircle2 className="w-5 h-5 text-red-500" />}
                     </div>
                  ...
                  <div className="p-4 pt-2 space-y-3 flex-grow flex flex-col">
                     ...
                     <p className="text-[11px] text-gray-500 leading-relaxed border-t border-white/5 pt-3 mt-auto">{item.desc}</p>
                  </div>
               </div>
             );
           })}
